...
io.on('connection', socket => {
  ...
  socket.on('register', user => {
    ...
    User.findOne({ email }).then(user => {
      if (user) {
        return socket.emit('RegError', {
          error: `Error: User with this email has already exists`
        });
      } else {
  ...
  socket.on('login', user => {
    ...
    User.findOne({ email }).then(user => {
      if (!user) {
        return socket.emit('LogError', {
          error: `Error: User with this email doesn't exists`
        });
      }
      ...
      bcrypt.compare(password, user.password, function(err, result) {
        if (result) {
          jwt.sign(
            ...
            (err, token) => {
              if (err) {
                res.json('LogError', { error: `Error: ${err}` });
              }
